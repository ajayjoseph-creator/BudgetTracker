  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
        <button
          onClick={handleBack}
          className="flex items-center gap-2 px-4 py-2 bg-white shadow rounded hover:bg-gray-100 transition"
        >
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded shadow hover:bg-green-600 transition"
        >
          ...
          <thead className="bg-gray-100">
            <tr>
              {["Category", "Color", "Limit", "Spent", "Remaining", "Actions"].map((h) => (
                <th key={h} className="px-6 py-3 text-left text-gray-700 uppercase tracking-wider text-sm">
                  {h}
                </th>
              ))}
            </tr>
            ...
            {summary.map((cat) => {
              ...
              return (
                <tr key={cat._id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">₹{cat.limit}</td>
                  <td className="px-6 py-4 text-gray-800">₹{cat.spent}</td>
                  <td className={`px-6 py-4 font-semibold ${remaining < 0 ? "text-red-600" : "text-green-600"}`}>
                    ₹{remaining}
                  </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-96 shadow-lg relative">
            ...
            <h2 className="text-xl font-bold mb-4">{modalMode === "add" ? "Add Category" : "Edit Category"}</h2>
            <div className="flex flex-col gap-3">
              <input
                ...
                className="border rounded px-3 py-2 focus:ring-1 focus:ring-green-500 focus:outline-none"
              />
              <input
                type="color"
                value={catColor}
